...
export const registerNewAdmin = catchAsyncErrors(async(req,res,next)=>{
    if(!req.files ||Object.keys(req.files).length===0){
        return next(new ErrorHandler("Please upload a file for the avatar.",400));
    }
    ...
    const alreadyRegistered = await User.findOne({email , accountVerified:true});
    ...
    if(!myCloud || myCloud.error){
        console.error("Cloudinary error: ",myCloud.error);
